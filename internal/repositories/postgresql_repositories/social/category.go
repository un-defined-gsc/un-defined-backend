...
func (r *CategoryRepository) GetByName(ctx context.Context, name string) (category domains.CategoryDTO, err error) {
	query := `SELECT id FROM t_categories WHERE name = $1`
	err = r.dbpool.QueryRow(ctx, query, name).Scan(&category.ID)
	return
}
